...
class Slider {
  constructor(options = {}) {
    ...
    // Touch state
    this._tx = 0; this._ty = 0; this._tdx = 0;
    this._isH = null; this._drag = false;

    // Mouse state
    this._md = false; this._mx = 0; this._mdx = 0;
  ...
  goTo(t) {
    if (this.isAnimating || t === this.currentIndex || t < 0 || t >= this.totalSlides) return;
  ...
  _bindTouch() {
    this.track.addEventListener('touchstart', e => {
      ...
      this._tdx = 0; this._drag = true; this._isH = null;
    }, { passive: true });
    ...
    this.track.addEventListener('touchend', () => {
      if (!this._drag || !this._isH) { this._drag = false; return; }
      this._drag = false;
      this._resetDrag();
      const thr = this.track.offsetWidth * .22;
  ...
  _bindMouse() {
    this.track.addEventListener('mousedown', e => {
      this._md = true; this._mx = e.clientX;
      this._mdx = 0; this.track.style.cursor = 'grabbing';
    });
    ...
    window.addEventListener('mouseup', () => {
      if (!this._md) return;
      this._md = false; this.track.style.cursor = '';
      this._resetDrag();
      const thr = this.track.offsetWidth * .20;
  ...
  _applyDrag(dx) {
    const d = dx * .65;
    if (this.cards[this.currentIndex])     this.cards[this.currentIndex].style.transform     = `translateX(${d}px)`;
    if (this.cards[this.currentIndex - 1]) this.cards[this.currentIndex - 1].style.transform = `translateX(calc(-100% + ${d}px))`;
    if (this.cards[this.currentIndex + 1]) this.cards[this.currentIndex + 1].style.transform = `translateX(calc(100% + ${d}px))`;
  }
